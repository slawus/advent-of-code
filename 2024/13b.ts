function processInput(input: string) {
  ...
  return taskStr.map(str => {
    ...
    return {
      ...
      ay: Number(ay),
      by: Number(by),
      X: Number("10000000000000") + Number(X),
      Y: Number("10000000000000") + Number(Y),
    }
  })
...
function main(text: string) {
  ...
  const sum = moves.filter(m => m !== null).reduce((acc, [a, b]) => acc + BigInt(a) * BigInt(3) + BigInt(b), BigInt(0));
  return {
    sum
  }
}
