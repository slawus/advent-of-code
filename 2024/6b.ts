...
function evaluatePath(start: Point, d: DIRECTION, matrix: number[][]) {
  let position = start;
  let steps = 0;
  let direction = d;
  ...
  const loop = !isOutside(matrix, position)
...
function findLoopObstacles(start: Point, d: DIRECTION, originalMatrix: number[][]) {
  let position = start;
  let direction = d;
  let matrix = JSON.parse(JSON.stringify(originalMatrix)); // clone
  let count = 0;
  while (!alreadyVisited(matrix, position, direction)) {
    matrix[position.y][position.x] = matrix[position.y][position.x] | direction;
    ...
    if (isOutside(matrix, nextPosition)) {
      ...
      position = nextPosition;
      if(matrix[position.y][position.x] === 0) {
        console.log(count);
        const modifiedMatrix = JSON.parse(JSON.stringify(originalMatrix));
        modifiedMatrix[position.y][position.x] = -1;
        const { loop }  = evaluatePath(start, d, modifiedMatrix);
        if(loop) {
          count++;
        }
      }
...
function main(text: string) {
  ...
  return findLoopObstacles(start, d, matrix);
}
